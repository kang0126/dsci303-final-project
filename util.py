...
def feature_pre_process(tmdb):
    ...
    # DIRECTORS - *** NOT USED ***
    directors_df = pd.read_csv('./data/Top-Directors.csv', encoding="ISO-8859-1")
    directors = directors_df['Name'].tolist()

    # ACTORS - STAR VALUES
    acts_df = pd.read_csv('./data/Top-1000-Actors-Actresses.csv', encoding="ISO-8859-1")
    ...
    companies = ['paramount', 'mgm', 'disney', 'dreamworks', 'twentieth', 'universal', 'lionsgate', 'warner',
                 'columbia']
    ...
    genres_sorted = ['Drama', 'Comedy', 'Thriller', 'Action', 'Romance', 'Crime', 'Adventure', 'Science Fiction',
                     'Horror',
...
def get_boolean_revenue(tmdb):
    ...
    return np.apply_along_axis(classify, 1, revenue)
...
def exp_naive_bayes(X_train, y_train, X_test, y_test):
    print('=-=-=-= Naive Bayes =-=-=-=')
...
def choose_trees_plot(X_train, y_train, X_test):
    oob = []
...
def exp_random_forest(X_train, y_train, X_test, y_test, n_trees=16):
    print('=-=-=-= Random Forest =-=-=-=')
    y_hat_training, y_hat_test, obb_score, feature_importances = applyRandomForest(X_train, y_train, X_test, n_trees)
...
def exp_log_reg(X_train, y_train, X_test, y_test):
    print('=-=-=-= Logistic Regression =-=-=-=')
    y_hat = applyLogisticRegr(X_train, y_train, X_test)
    print('Logistic Regression test error: %g' % np.mean(y_test != y_hat))
...
##### VISUALIZATION #####
def plot_genre_box(genres_sorted, tmdb):
    genre_box_plot_data = []
